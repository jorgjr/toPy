...
def allocH1H2(ne,nauxIx,nauxIy,nauxIz,rmin,vifac,dim,t,cent):
    ...
    vr = np.min(vifac)
    for i in range(0,ne):
        rij, elemid = tree.query(tree.data[i], np.uint32((vmin/vr)*2), eps=0, p=2, distance_upper_bound=rmin)
...
def allocH1H2H3(mn,ne,nn,nauxIx,nauxIy,nauxIz,rmin,vifac,dim,nt,cent,nod_inrad):
    ...
    vr = np.min(vifac)
    for i in range(0,ne):
        rij, elemid = tree.query(cent[i], np.uint32((vmin/vr)*2), eps=0, p=2, distance_upper_bound=rmin)
        n = len(np.where(~np.isinf(rij))[0])
        rij = rij[:n]
        elemid = elemid[:n]
        sumf = 0
